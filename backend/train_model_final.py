...
def find_optimal_thresholds(y_true, y_pred_proba, class_weights=None):
    ...
    for i in range(n_classes):
        ...
        if i == 0:  # Zika
            ...
            # For Chikungunya, prioritize precision
            target_fpr = 0.2
...
def create_interaction_features(df):
    """Create enhanced interaction features for better disease discrimination."""
    ...
    # Enhanced Zika patterns
    # Primary Zika pattern (key symptoms)
    df["zika_primary_pattern"] = (
        (df["exantema"] * df["conjuntvit"]) * 4.0 +     # Strong co-occurrence
        (df["exantema"] > 0.7) * 2.0 +                  # Strong rash
        (df["conjuntvit"] > 0.7) * 2.0                  # Strong conjunctivitis
    ) / 8.0

    # Secondary Zika pattern (mild symptoms)
    df["zika_secondary_pattern"] = (
        (df["constitutional_severity"] < 0.5) * 2.0 +    # Mild constitutional
        (df["articular_severity"] < 0.5) * 2.0 +        # Mild joint
        (1 - df["hemorrhagic_severity"]) * 2.0          # No hemorrhagic
    ) / 6.0

    # Combined Zika pattern
    df["zika_specific_pattern"] = (
        df["zika_primary_pattern"] * 3.0 +              # Primary symptoms
        df["zika_secondary_pattern"] * 2.0 +            # Secondary symptoms
        (df["exantema"] > df["hemorrhagic_severity"]) * 1.5  # Rash > hemorrhagic
    ) / 6.5
...
def main():
    ...
    try:
        ...
        # Save Zika specialist
        specialist_path = os.path.join(ARTIFACTS_DIR, "zika_specialist.joblib")
        joblib.dump(zika_specialist, specialist_path)
        print(f"Saved Zika specialist model to {specialist_path}")
